...
import { Group, People, School } from '@mui/icons-material';
import { getMyCohorts, transformCohortToGroup, getGroupContentCount } from '@learner/utils/API/GroupService';
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        ...
        const activeGroups = response.result.filter((cohort: any) => 
        ...
        const groupsWithContentCount = await Promise.all(
          transformedGroups.map(async (group, index) => {
            console.log(`Processing group ${index + 1}:`, { id: group.id, name: group.name }); // Debug log
            const contentCount = await getGroupContentCount(group.id);
            console.log(`Group ${group.name} (${group.id}) has ${contentCount} content items`); // Debug log
            return {
              ...group,
        ...
        console.log('Final groups data:', groupsWithContentCount.map(g => ({ id: g.id, name: g.name, contentCount: g.contentCount }))); // Debug log
