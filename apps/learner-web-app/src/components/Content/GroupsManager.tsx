...
const GroupsManager: React.FC<GroupsManagerProps> = ({ isLoading = false }) => {
  ...
  const handleContentClick = (content: GroupContentItem) => {
    ...
    try {
      ...
      const mimeType = mimeTypeMap[content.type] || content.mimeType || 'application/vnd.ekstep.html-archive';
      
      if (CONTENT_DETAILS_TYPES.includes(mimeType)) {
        // Navigate to content details for course/collection content
        const contentDetailsUrl = `/content-details/${content.id}?activeLink=${encodeURIComponent(activeLinkUrl)}`;
        ...
        const playerUrl = `/player/${content.id}?activeLink=${encodeURIComponent(activeLinkUrl)}`;
